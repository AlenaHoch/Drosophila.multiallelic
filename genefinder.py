...
# python3 genefinder.py -b test.bed -r genomic.gff -o testout.txt

parser = argparse.ArgumentParser(prog="Usage :python3 genefinder.py -b linvilla.bed -r genes_annotations_612.gff3 -o genes_linvilla.txt ",description="The script will use the BED file and return a list of genes covering those regions", epilog="" )
...
for line in temp:
    ...
    if len(t)>>3:
        if t[5] =="gene":
            out=("%s\t%s\t%s\t%s\n")%(t[0],t[1],t[2],t[11])
            # NEW LINE FROM TONI
            out=out.split(';Name=')[1].split(';')[0]+'\n'
            # YOU MIGHT WANT TO REMOVE LOC....
