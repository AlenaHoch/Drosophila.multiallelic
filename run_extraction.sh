...
freebayes -f /vol/storage/alenas_stuff/bamfiles/dmel-all-chromosome-r6.12.fasta --pooled-continuous \
-r 2R -r 2L -r 3R -r 3L \
-L /vol/storage/alenas_stuff/bamfiles/linvilla/list.txt > variants_allc_pooled.vcf

echo "Compressing VCF-file\n"
bgzip variants_allc_pooled.vcf

echo "Creating index for compressed VCF-file\n"
tabix variants_allc_pooled.vcf.gz

echo "Extracting allele counts with bcftools\n"
bcftools query -f '%CHROM\t%POS\t%REF\t%ALT[\t%AD]\n' variants_allc_pooled.vcf.gz > allele_counts_snps_allc.tsv
...
}' allele_counts_snps_allc.tsv > filtered_sites_allc.tsv
